                ...
                <div class="box-body table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="data-table">
                        ...
                        @foreach($hours as $hour)
                            <tr>
                                <td><span class="badge bg-green">{{ $hour->program->name }}</span></td>
                                <td><span class="badge bg-yellow">{{ $hour->start }}</span></td>
                                <td><span class="badge bg-red">{{ $hour->end }}</span></td>
                                <td>
                                    <input type="hidden" id="program-{{ $hour->id }}" value="{{ $hour->program_id }}">
            ...
            <div class="modal-body">
                <form class="form-horizontal" role="form">
                    <div class="form-group">
                        <label class="col-lg-3 control-label" for="program">Periode Roster</label>
                        <div class="col-lg-5">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-horizontal" role="modal">
                    <div class="form-group">
                        <label class="col-lg-3 control-label" for="updated_program">Periode Roster</label>
                        <div class="col-lg-5">
                        	<input type="hidden" name="id" value="0">
                            <select class="form-control" name="updated_program">
                            	@foreach($programs as $program)
                                    <option value="{{ $program->id }}">{{ $program->name }}</option>
                                @endforeach
                            </select>
...
<script type="text/javascript">
    ...
    function showFormUpdate(id)
    {
        ...
        $('select[name=updated_program]').val($('#program-'+id).val());
    ...
    function create()
    {
        ...
        if(program != '0' && start != '' && end != '')
        {
            $.ajax({
                url:'jam',
                type:'POST',
                data:{program : program, start : start, end : end},
                success:function(){
                    window.location = "jam";
                }
            });
    ...
    function update()
    {
        ...
        var program = $('select[name=updated_program]').val();
