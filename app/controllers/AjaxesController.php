...
class AjaxesController extends \BaseController {
	...
	public function suggestIssue($course_id)
	{
		...
		$issue = Issue::where('project_id','=',Auth::user()->curr_project_id)
						->where('location_id','=',Auth::user()->location_id)
						->where('generation_id','=',$course->generation_id)
						->orderBy('issue','desc')
						->first();
		
		if($issue)
		{
			$last_issue = $issue->issue;
		}
		else
		{
			$last_issue = $course->generation->code.$course->project->code.$course->location->code."00";
		}
		
		return Response::json(array('issue' => $last_issue));
	}
